...
#    lapply(1:Nvar, function(i){
    for(i in 1:Nvar){
        ...
        if(length(per.target)==1){
            Nma <- as.integer(pars[[i]]$Nma)#MA model orders
            Nar <- as.integer(pars[[i]]$Nar)#AR model orders
            Inds <- as.integer(pars[[i]]$Inds)#whether to include activity indices/noise proxies: e.g. Inds=c(4,5,6)
        }
        per.type <- pars[[i]]$per.type##periodogram type
#        instrument <- paste(per.target,collapse='-')
        if(length(per.target)>1){
###if there are multiple data sets to be calculated, then combine them. 
            instrument <- 'combined'
...
###If per.type==BFP, then use BFP() from "periodoframe.R". 
...
#                             ofac=ofac,fmin=frange[1],fmax=frange[2],quantify=quantify)
                rv.ls <- BFP(t=tab[,1],y=y,dy=dy,
                             Nma=Nma,Nar=Nar,Inds=Inds,model.type='man',Indices=Indices,
                             ofac=ofac,fmin=frange[1],fmax=frange[2],quantify=quantify)
                ylab <- 'ln(BF)'
                name <- 'logBF'
            }else if(per.type=='MLP'){
###if MLP, use MLP() from periodoframe.R
#                if(length(per.target)>1){
...
#            tit <- paste('Periodogram:',per.type,'; Target:',instrument,'; Observable',yparx)
...
#    fname <- paste0(paste(per.target,collapse='_'),'_',paste(ypars,collapse='.'),'_',paste(per.type,collapse=''),'_MA',paste(Nma,collapse=''),'proxy',paste(Inds,collapse='.'),'_',Nsig.max,'sig_',paste(Pmaxs,collapse='d'),'d')
    fname <- paste0(paste(per.target,collapse='_'),'_',paste(ypars,collapse='.'),'_',paste(per.type,collapse=''),'_AR',paste(Nar,collapse=''),'proxy',paste(Inds,collapse='.'),'_',Nsig.max,'sig_',paste(Pmaxs,collapse='d'),'d')
...
per2D.data <- function(vars,per.par,data){
    ...
    if(length(per.target)>1){
        instrument <- 'combined'
        subdata <- lapply(1:length(per.target),function(j) data[[per.target[j]]])
        tmp <- combine.data(data=subdata,Ninds=Inds,Nmas=Nmas,Nars=Nars)
        tab <- tmp$cdata
        idata <- tmp$idata
        colnames(tab) <- colnames(data[[1]])[1:3]
    }else{
    ...
    dy <- tab[,3]
    if(length(per.target)==1){
        mp <- MP(t=t,y=y,dy=dy,Dt=Dt,nbin=Nbin,ofac=ofac,fmin=frange[1],fmax=frange[2],per.type=per.type,sj=0,Nma=Nma,Nar=Nar,Inds=Inds,Indices=Indices)
    }else{
        mp <- MP(t=t,y=y,dy=dy,Dt=Dt,nbin=Nbin,ofac=ofac,fmin=frange[1],fmax=frange[2],per.type=per.type,sj=0,Nma=0,Nar=0,Inds=0,Indices=Indices)
    }
    x2 <- mp$tmid
    y2 <- mp$P
    z2 <- mp$powers
...
#    fname <- paste0(paste(per.target,collapse='_'),'_MP_',paste(per.type,collapse=''),'_MA',paste(Nmas,collapse=''),'proxy',paste(Inds,collapse='.'))
    fname <- paste0(paste(per.target,collapse='_'),'_MP_',paste(per.type,collapse=''),'_AR',paste(Nars,collapse=''),'proxy',paste(Inds,collapse='.'))
    if(length(per.target)==1){
...
####calculate BF for AR model
#calcBF <- function(data,Nbasic,proxy.type,Nma.max,groups=NULL,Nproxy=NULL,Npoly=c(2,0)){
calcBF <- function(data,Nbasic,proxy.type,Nar.max,groups=NULL,Nproxy=NULL,Npoly=c(2,0)){
    t <- data[,1]
    y <- data[,2]
    dy <- data[,3]
    NI.max <- ncol(data)-3
...
#    }
    if(ncol(data)>3){
        out <- BFP.comp(t, y, dy, Nars=0:Nar.max,NI.inds=NI.inds,Indices=data[,4:ncol(data),drop=FALSE])
    }else{
        out <- BFP.comp(t, y, dy, Nars=0:Nar.max,NI.inds=0,Indices=NA)
    }   
...
#    return(list(Inds=NI.inds,Inds.opt=out$Inds,Nmas=0:Nma.max,Nma.opt=out$Nma,logBF=out$logBF,NI.opt=out$NI))
    return(list(Inds=NI.inds,Inds.opt=out$Inds,Nars=0:Nar.max,Nar.opt=out$Nar,logBF=out$logBF,NI.opt=out$NI))
}
